...
function App() {
  ...
  const [theme, setTheme] = useState(() => localStorage.getItem('OndaRAG_theme') || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'night' : 'day'));
  ...
  useEffect(() => localStorage.setItem('OndaRAG_theme', theme), [theme]);
  ...
  return (
    <main className={`app-shell ${theme === 'night' ? 'theme-night' : 'theme-day'}`}>
      <aside className="sidebar" aria-label="Workspace navigation">
        <div className="brand"><span className="brand-mark">V</span><span>OndaRAG</span></div>
        <div className="sidebar-section">
          <div className="section-label">Workspace</div>
          <button className="nav-item active" type="button" aria-current="page"><span className="nav-icon" aria-hidden="true">V</span> Assistant <span className="nav-count">1</span></button>
...
function Message({ message, autoTts }) {
  ...
  return <article className={`message ${assistant ? 'assistant' : 'user'}`}><div className="avatar" aria-hidden="true">{assistant ? 'V' : 'Y'}</div><div className="message-body"><span className="message-label">{assistant ? 'OndaRAG' : 'YOU'}</span><div className="bubble">{message.content}</div>{assistant && message.audio && <TtsControls src={message.audio} autoPlay={autoTts} />}</div></article>;
}
